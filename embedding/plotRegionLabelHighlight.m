function [] = plotRegionLabelHighlight(xx,seg_im,im_mask,indx)
% plotRegionLabel(xx,seg_im,im_mask)
% plot segmentation regions with labels

% numPoints = length(xx);
im = double(seg_im);
im_highlight = zeros(size(im));
for n = 1:length(indx)
    im_highlight = im_highlight+double(seg_im==indx(n));
end
im = double(im~=0)+im_highlight;
im(im_mask==0) = -1;
imagesc(xx,xx,2-im);
colormap(gca,gray);
caxis([-1 2])
