function [hofAll,hogAll,mbhxAll,mbhyAll] = extractTwDT_galois(movieParam,...
...
% initialization
hofAll = cell(numVideo,numPatch);
hogAll = cell(numVideo,numPatch);
mbhxAll = cell(numVideo,numPatch);
mbhyAll = cell(numVideo,numPatch);
...
for i = 1:numVideo
    ...
    's_' num2str(track_thresh) '_' infostr '/' trackInfo(i).name],'\t',2,0);
        ...
        crHog = dt_features(:,11+sFlow:10+sFlow+sHog);
        crHog = crHog(~any(isnan(crHog),2),:);

        crHof = dt_features(:,11+sFlow+sHog:10+sFlow+sHog+sHof);
